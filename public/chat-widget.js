(function () {
  ...
  async function send(forced) {
    ...
    try {
      // Safari fix: use text/plain to avoid CORS preflight (OPTIONS request)
      // application/json triggers preflight which Safari blocks unless server sends correct headers
      // text/plain is a "simple request" — bypasses preflight — works on ALL browsers including Safari
